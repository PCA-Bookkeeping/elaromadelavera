      ...
      <section className="section-padding">
        <div className="container mx-auto max-w-3xl space-y-8">
          {[
            ...
            <div key={s.title.en}>
              <h2 className="font-heading text-xl font-semibold mb-3">{s.title[t('returns.title') === 'Return Policy' ? 'en' : t('returns.title') === 'Polityka Zwrotów' ? 'pl' : 'es']}</h2>
              <p className="text-muted-foreground leading-relaxed">{s.text[t('returns.title') === 'Return Policy' ? 'en' : t('returns.title') === 'Polityka Zwrotów' ? 'pl' : 'es']}</p>
            </div>
          ))}
        </div>
      </section>
